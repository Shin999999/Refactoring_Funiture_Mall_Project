...
-- TB_SHOP_CART_ITEM 수정 // 11월 29일 V0.1.4 : 테이블 삭제
-- TB_SHOP_CART 수정 // 11월 30일 V0.1.5 : 컬럼 삭제
-- TB_ITEM_IMAGES 수정 // 11월 30일 V0.1.6
-- TB_ITEM_REVIEW 수정 // 11월 30일 v0.1.7
-- TB_ITEM_IMAGES 수정 // 11월 30일 v0.1.8 : 조인용 컬럼 추가
-- TB_ITEM_IMAGES DROP CREATE  막음 // 12월 2일  V0.1.9
-- TB_SHOP_ITEMS TOTAL_LIKE  막음 // 12월 2일  V0.2.0
-- TB_SHOP_CART ALL_PRICE 제거 // 12월 4일 V0.2.1
...
DROP SEQUENCE TB_SHOP_ITEM_INFO_SEQUENCE;
...
CREATE SEQUENCE TB_SHOP_ITEM_INFO_SEQUENCE START WITH 1 INCREMENT BY 1;
...
DROP TABLE TB_SHOP_ITEM_INFO CASCADE CONSTRAINT;
...
--     TOTAL_LIKES NUMBER(5) DEFAULT 0,
    ...
    CONSTRAINT FK_SHOP_ITEMS_UUID FOREIGN KEY (UUID) REFERENCES TB_SHOP_ITEM_IMAGES (UUID)
);
...
(
    ...
    CONSTRAINT FK_SHOP_CART_UUID FOREIGN KEY (UUID) REFERENCES TB_SHOP_ITEM_IMAGES (UUID)
);
...
(
    ORDER_NO             NUMBER(5) NOT NULL, -- TODO: 리뷰랑 연결 될 거 같은데 상황에 따라서 부탁
    ITEM_NO              NUMBER(5),
    CART_NO              NUMBER(10),
    UUID                 VARCHAR2(1000) NOT NULL,
    CREATE_DATE VARCHAR2(1000),
    UPDATE_DATE VARCHAR2(1000),
    DELETE_YN   VARCHAR2(1) DEFAULT 'N',
    CONSTRAINT PK_SHOP_ORDERS PRIMARY KEY (ORDER_NO), -- 주문번호 기본키
    CONSTRAINT FK_SHOP_ORDERS_ITEM FOREIGN KEY (ITEM_NO) REFERENCES TB_SHOP_ITEMS (ITEM_NO), -- 아이템 넘버 받은거
    CONSTRAINT FK_SHOP_ORDERS_CART FOREIGN KEY (CART_NO) REFERENCES TB_SHOP_CART (CART_NO),
    CONSTRAINT FK_SHOP_ORDERS_UUID FOREIGN KEY (UUID) REFERENCES TB_SHOP_ITEM_IMAGES (UUID)-- 아이템 넘버 받은거
);
...
CREATE TABLE TB_SHOP_ITEM_INFO
(
    ITEM_INFO_NO NUMBER(5) NOT NULL,
    ITEM_INFO_TITLE VARCHAR2(300),
    ITEM_BRAND VARCHAR2(100),
    ITEM_INFO1 VARCHAR2(1000),
    ITEM_INFO2 VARCHAR2(1000),
    ITEM_INFO3 VARCHAR2(1000),
    CREATE_DATE VARCHAR2(1000),
    UPDATE_DATE VARCHAR2(1000),
    DELETE_YN   VARCHAR2(1) DEFAULT 'N',
    CONSTRAINT PK_SHOP_ITEM_INFO PRIMARY KEY (ITEM_INFO_NO)
